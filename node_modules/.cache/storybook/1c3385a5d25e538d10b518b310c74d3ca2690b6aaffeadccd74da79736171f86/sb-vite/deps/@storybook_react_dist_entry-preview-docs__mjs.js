  ...
  try {
    switch (type.name) {
      ...
      case "arrayOf": {
        ...
        return compact != null && !Ht(compact) ? O(compact) : full ? O(short, full) : O(short);
      }
      case "func": {
        let { short, full } = generateType(type, extractedProp), summary = short, detail;
        return full && full.length < MAX_FUNC_LENGTH ? summary = full : full && (detail = toMultilineSignature(full)), O(summary, detail);
      }
      default:
        return null;
    }
...
function generateFunc2({ inferredType, ast }) {
  let { identifier } = inferredType;
  if (identifier != null) return O(getPrettyIdentifier(inferredType), generateCode(ast));
  let prettyCaption = generateCode(ast, true);
  return os(prettyCaption) ? O(FUNCTION_CAPTION, generateCode(ast)) : O(prettyCaption);
}
...
function getPropDefs(component, section) {
  ...
  !Ki(component) && !component.propTypes && isMemo2(component) && (processedComponent = component.type);
...
var renderJsx = (code, options) => {
  ...
  typeof (options == null ? void 0 : options.displayName) == "string" ? displayNameDefaults = { showFunctions: true, displayName: () => options.displayName } : displayNameDefaults = { displayName: (el) => {
    var _a;
    return el.type.displayName ? el.type.displayName : $i(el.type, "displayName") ? $i(el.type, "displayName") : ((_a = el.type.render) == null ? void 0 : _a.displayName) ? el.type.render.displayName : typeof el.type == "symbol" || el.type.$$typeof && typeof el.type.$$typeof == "symbol" ? getReactSymbolName(el.type) : el.type.name && el.type.name !== "_default" ? el.type.name : typeof el.type == "function" ? "No Display Name" : isForwardRef2(el.type) ? el.type.render.name : isMemo2(el.type) ? el.type.type.name : el.type;
  } };
...
var skipJsxRender = (context) => {
  ...
  return (sourceParams == null ? void 0 : sourceParams.type) === sT.DYNAMIC ? false : !isArgsStory || (sourceParams == null ? void 0 : sourceParams.code) || (sourceParams == null ? void 0 : sourceParams.type) === sT.CODE;
};
var isMdx = (node) => {
  var _a, _b;
...
var parameters = { docs: { story: { inline: true }, extractArgTypes, extractComponentDescription: $w } };
